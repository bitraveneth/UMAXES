...
export default function DocumentDownloadMenu({
  ...
  if (admin) {
    return (
      <div
        className="inline-flex overflow-hidden rounded-md border border-[var(--admin-border)] bg-white"
        onClick={(e) => e.stopPropagation()}
      >
        <a
          href={pdf}
          className="inline-flex items-center gap-1.5 border-r border-[var(--admin-border)] px-2.5 py-1.5 text-xs font-semibold text-[var(--admin-text)] transition hover:bg-[var(--admin-brand-50)]"
          title="Download PDF"
        >
        ...
        <a
          href={xlsx}
          className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold text-[var(--admin-text)] transition hover:bg-[var(--admin-brand-50)]"
